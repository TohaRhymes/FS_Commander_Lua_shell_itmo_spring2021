...
int
find_node_by_name(struct ntfs_sb_info *fs, char *path, struct ntfs_inode **start_node, struct ntfs_find_info **result) {
    ...
    if (err_f != -1) {
        ...
        while (tmp != NULL) {
            if (tmp->type & MFT_RECORD_IS_DIRECTORY) {
                ...
                n = files_createNode(tmp->filename, "DIR");
//                strcpy(cur->file_type, "DIR\0");
            } else {
                sprintf(result, "FILE\t%s\n", tmp->filename);
                n = files_createNode(tmp->filename, "FILE");
//                strcpy(cur->file_type, "FILE\0");
            }
            // Insert node at end of list.
            files_insert(&tail->next, n);
            // Update tail.
            tail = n;
...
int copy(struct ntfs_sb_info *fs, struct ntfs_inode *node, char *out_path) {
    ...
    if (!(node->type & MFT_RECORD_IS_DIRECTORY)) {
        ...
        if (chunk->resident) {
            ...
            unsigned long size;
            bool flag = true;
            while (read_block_file(&chunk, fs) == 0 && flag) {
